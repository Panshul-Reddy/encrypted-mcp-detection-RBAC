...
async def main() -> None:
    ...
    print("[noise] Patterns: rest_polling | websocket_stream | chunked_stream |"
          " sse_stream | burst_requests")

    # Stagger pattern startup to avoid simultaneous initialization at time zero.
    patterns = [
        ("rest_polling",     pattern_rest_polling),
        ("websocket_stream", pattern_websocket_stream),
        ("chunked_stream",   pattern_chunked_stream),
        ("sse_stream",       pattern_sse_stream),
        ("burst_requests",   pattern_burst_requests),
    ]
