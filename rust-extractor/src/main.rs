...
async fn main() -> Result<()> {
    ...
    let log_path = PathBuf::from("../logs/encrypted_rbac_audit.jsonl");
    tokio::spawn(async move {
        ...
        loop {
            if let Ok(mut file) = tokio::fs::File::open(&log_path).await {
                ...
                file.seek(std::io::SeekFrom::Start(last_pos)).await.unwrap();
                let mut buf = String::new();
                if file.read_to_string(&mut buf).await.unwrap_or(0) > 0 {
                    last_pos = file.stream_position().await.unwrap_or(last_pos);
                    let mut lock = tui_state_for_log.lock().unwrap();
                    for line in buf.lines() {
                        if line.trim().is_empty() { continue; }
                        if let Ok(entry) = serde_json::from_str::<ProxyLogEntry>(line) {
                            if let (Some(cp), Some(dp)) = (entry.client_port, entry.dst_port) {
                                // Match the flow key generated by the extractor: "src_ip:src_port -> dst_ip:dst_port"
                                // Note: Extractor uses "127.0.0.1" for dst_ip usually
                                let key = format!("{}:{} -> 127.0.0.1:{}", entry.client_ip, cp, dp);
                                lock.meta_map.insert(
                                    key,
                                    live_analyzer::tui::FlowMeta {
                                        server_name: entry.server,
                                        role: entry.role,
                                        accessed: entry.tool.filter(|s| !s.is_empty()),
