...
async fn main() -> Result<()> {
    ...
    // ── Background Task: Tail Proxy Audit Log ──
    ...
    // Auto-resolve absolute path to logs folder relative to the rust-extractor directory or project root
    let log_path = {
        let exe_path = std::env::current_exe().unwrap_or_else(|_| PathBuf::from("."));
        let mut dir = exe_path.parent().unwrap_or(std::path::Path::new(".")).to_path_buf();
        for _ in 0..3 {
            if dir.join("start_demo.ps1").exists() {
                break;
            }
        ...
        dir.join("logs").join("encrypted_rbac_audit.jsonl")
    };
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            if file.seek(std::io::SeekFrom::Start(last_pos)).await.is_err() { continue; }

            let mut buf = String::new();
            let bytes_read = file.read_to_string(&mut buf).await.unwrap_or(0);
            if bytes_read == 0 { continue; }
            last_pos += bytes_read as u64;

            let mut lock = tui_state_for_log.lock().unwrap();
            for line in buf.lines() {
                if line.trim().is_empty() { continue; }
                if let Ok(entry) = serde_json::from_str::<ProxyLogEntry>(line.trim()) {
                    if let (Some(cp), Some(dp)) = (entry.src_port, entry.dst_port) {
        ...
        match client.predict(&features, &src_ip_str, flow.key.src_port, &dst_ip_str, flow.key.dst_port, gt_str).await {
            Ok((prediction, latency)) => {
                let classified = ClassifiedFlow {
                    ...
                    canonical_key: flow.key.canonical_key(),
                    label: prediction.label as u8,
                    proba_mcp: prediction.proba[1..7].iter().sum(),
